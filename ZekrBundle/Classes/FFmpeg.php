<?php
//https://github.com/dmongeau/FFMPEG

namespace ZekrBundle\Classes;
...
class FFmpeg
{
    ...
    public function setFFmpegBin($bin)
    {
        self::$_config['bin'] = $bin;
    }
        ...
        foreach ($output as $line) {
            ...
            if (!$metaHeader) {
                if (preg_match('/^\s{2}Duration\: ([0-9\:\.]{11})/', $line, $matches)) {
                    ...
                    if (sizeof($metadata['format']) == 1)
                        $metadata['format'] = $metadata['format'][0];
    ...
    public function getThumbnail($path, $start = 1)
    {
        if ($start == 'half') {
            $metadata = $this->getMetadata();
    ...
    protected function _execute($output = null)
    {
        ...
        if ($this->hasOptions('main'))
            $parts[] = $this->_prepareOptions('main');
        if ($this->hasOptions('in'))
            $parts[] = $this->_prepareOptions('in');
        $parts[] = '-i ' . escapeshellarg($this->getInputFile());

        if ($this->hasOptions('out'))
            $parts[] = $this->_prepareOptions('out');
        if (isset($output))
    ...
    public function _exec($command)
    {
        ...
        $descriptorspec = array(
            0 => array("pipe", "r"),
            1 => array("pipe", "w"),
            2 => array("file", __DIR__.'/../../../var/logs/ffmpeg-error-output.txt', "a")
        );


        $cwd = dirname($this->_inputFile).'/';

        //$cwd = 'D:/www/www/medialib/dev/web/bundles/rawafedmedialib/media';
    ...
    protected function _catchError($code)
    {
        ...
        switch ((int) $code) {
            ...
            case 130:
                ...
                break;
            default:
                throw new Exception('An error occurred', $code);
                break;
        }
    }
    ...
    public static function setConfig($name, $value = null)
    {
        ...
        if (!isset($value) && is_array($name))
            self::$_config = $name;
        else if (isset($value) && !is_array($name))
